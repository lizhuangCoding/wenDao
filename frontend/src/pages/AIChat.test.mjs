...
const loadPageSource = async (name) => {
...
test('AIChat uses expressive agent status indicators while processing', async () => {
  ...
  assert.match(runtimeSource, /selectFeaturedAgentStep/);
...
test('AIChat page delegates local UI state and header controls to focused modules', async () => {
  ...
  assert.match(source, /useAIChatRuntime/);
...
test('AIChat page delegates store selection and runtime derivations to a focused runtime hook', async () => {
  const [source, runtimeSource] = await Promise.all([
    loadAIChatSource(),
    loadPageSource('useAIChatRuntime'),
  ]);
  ...
  assert.match(source, /useAIChatRuntime/);
  assert.doesNotMatch(source, /useChatStore/);
  assert.match(runtimeSource, /useChatStore/);
  assert.match(runtimeSource, /questionNavItems/);
  assert.match(runtimeSource, /featuredAgentStep/);
});
